...
void printUsage(const char* progName) {
  ...
  std::cout << "  display <count>              Display first N entries from the table"
            << std::endl;
  std::cout << "  display-buckets [max]        Display first entry from each bucket"
            << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "HashChainTable Verifier (Implementation 3: In-Memory Dynamic Nonce)" << std::endl;
  ...
  hashchain::VerifierV3 verifier(bucketDir);
  ...
  if (command == "display") {
    ...
    if (argc < 6) {
      std::cerr << "Error: find command requires target_hash, difficulty, and max_matches"
                << std::endl;
      printUsage(argv[0]);
      return 1;
    }
