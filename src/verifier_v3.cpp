...
VerifierV3::VerifierV3(const std::string& bucketDir) : bucketDir_(bucketDir), effectiveNonceSize_(0) {
...
bool VerifierV3::loadMetadata() {
  ...
  if (version != 3) {
    std::cerr << "Error: Metadata file is not for Implementation 3 (version=" << version << ")"
              << std::endl;
    return false;
  }
  ...
  // Read effective nonce size (CRITICAL!)
  uint32_t effectiveNonce;
  metaFile.read(reinterpret_cast<char*>(&effectiveNonce), sizeof(effectiveNonce));
  effectiveNonceSize_ = static_cast<size_t>(effectiveNonce);

  std::cout << "Loaded metadata: effective nonce size = " << effectiveNonceSize_ << " bytes"
            << std::endl;
...
std::array<uint8_t, Config::hashSize> VerifierV3::computeFullHash(const Nonce& nonce) {
...
std::vector<Nonce> VerifierV3::readBucket(size_t bucketId) {
  ...
  // NOTE: File contains only effectiveNonceSize_ bytes per entry
  nonces.resize(meta.entryCount);
  for (size_t i = 0; i < meta.entryCount; ++i) {
    // Initialize nonce to zero
    nonces[i].fill(0);

    // Read only effectiveNonceSize_ bytes from file
    dataFile.read(reinterpret_cast<char*>(nonces[i].data()), effectiveNonceSize_);

    // Remaining bytes stay zero (if effectiveNonceSize_ < Config::nonceSize)
  }
...
std::string VerifierV3::nonceToHex(const Nonce& nonce) const {
  std::ostringstream oss;
  oss << std::hex << std::setfill('0');
  // Only display the bytes that were actually stored
  for (size_t i = 0; i < effectiveNonceSize_ && i < Config::nonceSize; ++i) {
    oss << std::setw(2) << static_cast<int>(nonce[i]);
  }
  return oss.str();
}

std::string VerifierV3::hashToHex(const std::array<uint8_t, Config::hashSize>& hash) const {
  std::ostringstream oss;
  oss << std::hex << std::setfill('0');
...
bool VerifierV3::hashesMatchDifficulty(const std::array<uint8_t, Config::hashSize>& hash1,
...
size_t VerifierV3::getBucketIdFromHash(const std::array<uint8_t, Config::hashSize>& hash) const {
  size_t bucketId = 0;
...
void VerifierV3::displayFirstEntries(size_t count) {
  std::cout << "\n=== Displaying First " << count << " Entries (Implementation 3) ===" << std::endl;
...
void VerifierV3::displayFirstFromEachBucket(size_t maxBuckets) {
  std::cout << "\n=== Displaying First Entry From Each Bucket (Implementation 3) ==="
            << std::endl;
  std::cout << "Effective nonce size: " << effectiveNonceSize_ << " bytes" << std::endl;
...
size_t VerifierV3::findMatches(const std::array<uint8_t, Config::hashSize>& targetHash,
                               size_t difficulty, size_t maxMatches) {
  std::cout << "\n=== Finding Matches for Target Hash (Implementation 3) ===" << std::endl;
  std::cout << "Effective nonce size: " << effectiveNonceSize_ << " bytes" << std::endl;
